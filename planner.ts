...
//   system: the endpoint passes in the topics of the client's last ~20 posts, and
...
export const PLANNER_SYSTEM = `You are the content planner for a dental clinic's social media. You produce a weekly content plan: exactly three post topics, one for each pillar (Monday, Wednesday, Friday).
...
- Stay within scope of general dentistry patient education, clinic trust-building, and light engagement. No specialised medical advice, no diagnosis, no treatment guarantees.`;
...
export function buildPlannerUser(input: {
  ...
  pillarWed: string;
  pillarFri: string;
  recentTopics: string[]; // topics of the client's last ~20 posts
}): string {
  const recent =
